...
from app.utils.helper import calculate_bw_ratio, choice_generator, is_circle_inside
...
def align_crop(image_buffer, src_markers):
    ...
    src_markers = [
        element for element in src_markers if element["id"] in [3, 5, 7, 9, 11]
    ]
    ...
    for index, src_marker in enumerate(src_markers):
        dest_marker = next(
            (
                dest_marker
                for dest_marker in dest_markers
                if dest_marker["id"] == src_marker["id"]
            ),
            None,
        )
